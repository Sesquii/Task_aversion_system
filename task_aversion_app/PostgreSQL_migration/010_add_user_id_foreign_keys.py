#!/usr/bin/env python3
"""
PostgreSQL Migration 010: Add user_id Foreign Keys to Existing Tables
...
PostgreSQL-specific:
- Uses proper foreign key constraints with ON DELETE CASCADE or RESTRICT
- Adds indexes on user_id columns for query performance
- Handles VARCHAR to INTEGER conversion for existing user_id columns
...
def migrate():
    ...
    print("=" * 70)
    print("PostgreSQL Migration 010: Add user_id Foreign Keys")
    print("=" * 70)
    ...
    tables_with_new_user_id = [
        ...
        # emotions table may be shared or user-specific - check if it exists
    ]
    
    # Tables that need user_id converted from VARCHAR to INTEGER
    tables_with_varchar_user_id = [
        {
            'table': 'user_preferences',
            'description': 'User preferences table - convert VARCHAR user_id to INTEGER',
            'is_primary_key': True,  # user_id is primary key in this table
            'add_index': False  # Already indexed as PK
        },
        {
            'table': 'survey_responses',
            'description': 'Survey responses table - convert VARCHAR user_id to INTEGER',
            'is_primary_key': False,
            'add_index': True
        },
        {
            'table': 'popup_triggers',
            'description': 'Popup triggers table - convert VARCHAR user_id to INTEGER',
            'is_primary_key': False,
            'add_index': True
        },
        {
            'table': 'popup_responses',
            'description': 'Popup responses table - convert VARCHAR user_id to INTEGER',
            'is_primary_key': False,
            'add_index': True
        },
    ]
    ...
    try:
        with get_session() as session:
            ...
            for table_def in tables_with_new_user_id:
                ...
                try:
                    # Add nullable INTEGER user_id column
                    alter_sql = f'ALTER TABLE "{table_name}" ADD COLUMN user_id INTEGER REFERENCES users(user_id) ON DELETE CASCADE'
            ...
            # Step 2: Convert VARCHAR user_id to INTEGER for tables that have VARCHAR
            print("\nStep 2: Converting VARCHAR user_id to INTEGER for existing tables...")
            print("-" * 70)
            print("NOTE: This is a complex migration. For tables with existing VARCHAR user_id,")
            print("we will add a new INTEGER user_id column and keep VARCHAR temporarily.")
            print("A separate data migration script will populate INTEGER user_id from VARCHAR.")
            print()
            ...
            for table_def in tables_with_varchar_user_id:
                ...
                if not column_exists(table_name, 'user_id'):
                    ...
                    try:
                        if table_def.get('is_primary_key', False):
                            # This is more complex - primary key conversion needs special handling
                            ...
                            alter_sql = f'ALTER TABLE "{table_name}" ADD COLUMN user_id_new INTEGER REFERENCES users(user_id) ON DELETE CASCADE'
                            session.execute(text(alter_sql))
                            
                            if table_def.get('add_index', False):
                                index_sql = f'CREATE INDEX IF NOT EXISTS idx_{table_name}_user_id ON "{table_name}"(user_id_new)'
                                session.execute(text(index_sql))
                            
                            session.commit()
                            print(f"   [OK] Added INTEGER 'user_id_new' column to '{table_name}' (migration column)")
                            print(f"        Note: Original VARCHAR user_id kept for data migration")
                            converted_count += 1
                    except Exception as e:
                        session.rollback()
                ...
                if col_type and 'VARCHAR' in col_type.upper() or 'TEXT' in col_type.upper():
                    ...
                    try:
                        ...
                        print(f"        Note: Original VARCHAR user_id column kept temporarily for data migration")
                        ...
                        print(f"   [ERROR] {error_msg}")
                        errors.append(error_msg)
                elif col_type and 'INTEGER' in col_type.upper() or 'SERIAL' in col_type.upper():
                    print(f"   [OK] '{table_name}' already has INTEGER user_id (type: {col_type})")
                    # Check if foreign key constraint exists
                    inspector = inspect(engine)
                    foreign_keys = inspector.get_foreign_keys(table_name)
                    has_fk = any(fk['referred_table'] == 'users' for fk in foreign_keys)
                    if not has_fk:
                        print(f"   [INFO] Adding foreign key constraint to '{table_name}'...")
                        try:
                            fk_sql = f'ALTER TABLE "{table_name}" ADD CONSTRAINT fk_{table_name}_user_id FOREIGN KEY (user_id) REFERENCES users(user_id) ON DELETE CASCADE'
                            session.execute(text(fk_sql))
                            session.commit()
                            print(f"   [OK] Added foreign key constraint to '{table_name}'")
                        except Exception as e:
                            session.rollback()
                            print(f"   [WARNING] Could not add foreign key constraint: {e}")
                    else:
                        print(f"   [OK] Foreign key constraint already exists")
                    skipped_count += 1
                else:
            ...
            if added_count > 0 or converted_count > 0:
                ...
                print("\nNext steps:")
                print("  1. For tables with 'user_id_new' columns: Run data migration to populate from VARCHAR user_id")
            ...
            print("\nNOTE: Tables with VARCHAR user_id columns will need a data migration script")
